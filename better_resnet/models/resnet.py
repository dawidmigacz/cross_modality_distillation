...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, drop_prob=0.1, block_size=3):
        super(BasicBlock, self).__init__()
        self.db1 = DropBlock2D(drop_prob=drop_prob, block_size=block_size)
        self.conv1 = nn.Conv2d(
            in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.db2 = DropBlock2D(drop_prob=drop_prob, block_size=block_size)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, drop_prob=0.13, block_size=3):
    ...
    def _make_layer(self, block, planes, num_blocks, stride, drop_prob=0.12, block_size=3):
...
def ResNet18(dropblock_prob=0.11, dropblock_size=3):
    return ResNet(BasicBlock, [2, 2, 2, 2], drop_prob=dropblock_prob, block_size=dropblock_size)
